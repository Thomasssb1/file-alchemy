...
def test_adding_file_populates_file_list(
...
def test_adding_duplicate_files_is_ignored(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_multiple_files_all_appear_in_list(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_adding_files_keeps_existing_selection(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_video_file_shows_cross_format_section(
    page: MediaPage, tmp_path: Path
) -> None:
    _add_files(page, "clip.mp4", tmp_path=tmp_path)
    ...
    cross_headers = [
        ...
        and "cross" in model.item(i).text().lower()
    ]
    assert cross_headers, "Expected at least one '(cross-format)' header for video input"


def test_all_header_items_are_disabled(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_initial_selection_is_enabled_item(
    page: MediaPage, tmp_path: Path
) -> None:
    _add_files(page, "clip.mp4", tmp_path=tmp_path)
...
def test_same_category_format_selectable(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_cross_category_format_selectable(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_start_conversion_skips_header_text(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_drop_zone_callback_wiring(
    page: MediaPage, tmp_path: Path
) -> None:
...
def test_pick_output_dir_updates_label(
    page: MediaPage, tmp_path: Path
) -> None:
    with patch(
